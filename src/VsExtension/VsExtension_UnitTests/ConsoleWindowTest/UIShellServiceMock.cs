...
namespace VsExtension_UnitTests.ConsoleWindowTest
{
    static class UIShellServiceMock
    {
        ...
        /// <summary>
        /// Get an IVsUiShell that implement CreateConsoleWindow
        /// </summary>
        /// <returns>uishell mock</returns>
        internal static BaseMock GetUiShellInstanceCreateToolWin()
        {
            BaseMock uiShell = GetUiShellInstance();
            string name = string.Format("{0}.{1}", typeof(IVsUIShell).FullName, "CreateConsoleWindow");
            uiShell.AddMethodCallback(name, new EventHandler<CallbackArgs>(CreateConsoleWindowCallBack));

            return uiShell;
        }

        /// <summary>
        /// Get an IVsUiShell that implement CreateConsoleWindow (negative test)
        /// </summary>
        /// <returns>uishell mock</returns>
        internal static BaseMock GetUiShellInstanceCreateToolWinReturnsNull()
        {
            BaseMock uiShell = GetUiShellInstance();
            string name = string.Format("{0}.{1}", typeof(IVsUIShell).FullName, "CreateConsoleWindow");
            uiShell.AddMethodCallback(name, new EventHandler<CallbackArgs>(CreateConsoleWindowNegativeTestCallBack));
        ...
        #region Callbacks
        private static void CreateConsoleWindowCallBack(object caller, CallbackArgs arguments)
        {
        ...
        private static void CreateConsoleWindowNegativeTestCallBack(object caller, CallbackArgs arguments)
        {
            arguments.ReturnValue = VSConstants.S_OK;
